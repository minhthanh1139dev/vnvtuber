...
const LOG = "[QUEUE:CONSUMER]";

let webhookWorker = null;
let subscriptionWorker = null;
let started = false;

function startWorkers(connection) {
  if (started) return;
  ...
  started = true;
  logger.info(
    `${LOG} Started (webhook=${config.workers.webhookConcurrency}, subscription=${config.workers.subscriptionConcurrency})`,
  );
}
...
module.exports = new ChannelConsumer();
